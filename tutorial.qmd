        ...
        *,
        ...
        def _pwo_cost_and_grad(params, batch, log_old, adv_R, adv_I):
            def loss_fn(p):
                ...
                raw_dif = jnp.imag(log_new) - jnp.imag(log_old)
                dif = 2 * jnp.atan2(jnp.sin(raw_dif), jnp.cos(raw_dif))
                dif_clip = jnp.clip(dif, -i_clip_eps, i_clip_eps)
                loss_I = jnp.mean(sg_ratio * jnp.maximum(dif * adv_I, dif_clip * adv_I))
