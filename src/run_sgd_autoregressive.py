...
class GRULayer(nn.Module):
    ...
    def __call__(self, x: jax.Array) -> jax.Array:
        ...
        _, h = gru(carry0, x)  # (B, T, hidden_size)
        return h


class RecurrentAR(nk.models.AbstractARNN):
    hilbert: object
    embed_dim: int = 32
    rnn_hidden: int = 128
    head_hidden: int = 128
    ...
    def conditionals_log_psi(self, inputs: jax.Array) -> jax.Array:
        ...
        # NetKet Spin(1/2) configs are usually in {-1, +1}; map to {0, 1}.
        spin_tokens = (inputs > 0).astype(jnp.int32)  # (B, N)

        bos_token = jnp.full((B, 1), 2, dtype=jnp.int32)  # 0/1 = spins, 2 = BOS
        prev_tokens = jnp.concatenate([bos_token, spin_tokens[:, :-1]], axis=1)  # (B, N)

        tok_emb = nn.Embed(num_embeddings=3, features=self.embed_dim)
        x = tok_emb(prev_tokens)  # (B, N, embed_dim)

        if self.use_site_embedding:
            pos_ids = jnp.arange(N, dtype=jnp.int32)[None, :]  # (1, N)
            pos_emb = nn.Embed(num_embeddings=N, features=self.embed_dim)
            x = x + pos_emb(pos_ids)  # broadcast over batch

        x = nn.Dense(self.rnn_hidden)(x)
        x = nn.tanh(x)

        for _ in range(self.n_gru_layers):
            h = GRULayer(hidden_size=self.rnn_hidden)(x)
            x = x + h  # simple residual connection

        x = nn.Dense(self.head_hidden)(x)
        x = nn.gelu(x)
        x = nn.Dense(self.head_hidden)(x)
        x = nn.gelu(x)

        logits = nn.Dense(self.hilbert.local_size)(x)  # (B, N, 2)
        logp = jax.nn.log_softmax(logits, axis=-1)

        return logp / float(self.machine_pow)
...
def make_local_energy_fn(model: nn.Module, H):
    def local_energy(params, configs):
        ...
        ratios = jnp.exp(logpsi_xp - logpsi_x[:, None]) # (B,K)
        e_loc = jnp.sum(mels * ratios, axis=1) # (B,)
...
def main(cfg: Args) -> None:
    ...
    def collect_batch(old_params, sampler_state):
        ...
        configs_flat = configs.reshape(-1, N)  # (B, N)

        old_logpsi = model.apply({"params": old_params}, configs_flat)  # (B,)
        old_logp = cfg.machine_pow * jnp.real(old_logpsi)
        old_logp = jax.lax.stop_gradient(old_logp)

        e_loc = local_energy(old_params, configs_flat)  # (B,)
    ...
    def loss_on_batch(params, batch):
        ...
        new_logpsi = model.apply({"params": params}, configs_flat)  # (B,)
        ...
        ppo_loss = jnp.mean(jnp.maximum(surrogate_1, surrogate_2))
    ...
    def eval_chunk_sums(params, configs):
        e_loc = local_energy(params, configs)          # (B,)
    ...
    def evaluate(state):
        ...
        while n_remaining > 0:
            eval_vstate.sample()  # fresh chunk
    ...
    def train_iter(state, sampler_state):
        ...
        def epoch_step(state, _):
            (loss, metrics), grads = jax.value_and_grad(
                loss_on_batch, argnums=0, has_aux=True
            )(state.params, batch)
